...
//        caches.remove(0);//释放空间，重新添加 ,如果不释放空间，会报错：java.lang.OutOfMemoryError: Java heap space 【这里这样做，主要为了防止数组对象实际大小超过堆大小】
    ...
    public static void getJvmInfo() {
        ...
        System.out.println("运行时总内存"+Runtime.getRuntime().totalMemory()/1024/1024 + "Mb");
        System.out.println("运行时空闲内存"+Runtime.getRuntime().freeMemory()/1024/1024 + "Mb");
        System.out.println("运行时最大内存"+Runtime.getRuntime().maxMemory()/1024/1024 + "Mb");
